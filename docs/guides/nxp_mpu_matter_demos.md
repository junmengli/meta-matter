...
- You can refer to the [Yocto images and demos build guide](quick_start.md) to build Yocto images and demos for i.MX Matter devices.
...
Figure Matter with OTBR network topology diagram for i.MX93 FRDM, i.MX93 EVK, i.MX91 EVK, i.MX91 QSB, i.MX91 FRDM, and i.MX95 15x15 EVK
 ...
 <img src="../../_static/images/mpu/matter_demos/imx8mm_imx6ull_imx8ulp-otbr.png" width = "500"/>

Figure Matter with OTBR network topology diagram for i.MX 8M Mini EVK or i.MX6ULL EVK
...
For i.MX93 FRDM, it is essential to modify the fdtfile for it to work properly. Enter U-Boot mode and run the following commands to set the fdtfile, save the fdtfile setting, and boot the board.
...
**A hardware config needed on i.MX95 15x15 EVK: turn on the SW10 Pin1 to “ON”, which enable the SPI CS for IW612.**
...
#### For i.MX 8M Mini EVK + 88W8987 + K32W platform or i.MX8ULP EVK + IW416 + K32W platform:
...
For i.MX6ULL, it is mandatory to change fdt_file to setup WiFi and BT. Enter U-Boot mode and run the follow commands, then reboot the board.
...
The i.MX6ULL OTBR setup commands are the same with the i.MX8M Mini commands.
...
***Note: please use the ot-ctl-iwxxx instead of ot-ctl on the i.MX9 series platform.***
...
Since chip-tool-trusty can run on the i.MX 8M Mini EVK platform, the chip-tool in the above commands can replace chip-tool-trusty. **Before running chip-tool-trusty, you should use to the [enable command](#enable-the-secure-storage-service) to enable the secure storage service.**
...
**Note: Please [check SPI device](#check-spi-device) to identify the SPI device name and[check GPIO device](#check-gpio-device) to determine gpio-reset-device and gpio-int-device used in the RadioURL option of otbr-agent-iwxxx.**
...
***Note: please use the ot-client-iwxxx instead of ot-client-ctl on the i.MX9 series platform.***
...
The network topology diagram by using ble-wifi commissioning is shown below.
 ...
 <img src="../../_static/images/mpu/matter_demos/ble-wifi.png" width = "500"/>
...
Step 2. Setup BT and connected to a WiFi AP.
...
<a name="load-the-wi-fibt-firmware"></a>
...
>
**Note: Before running imx-thread-br-app, ensure the following prerequisites are completed on the end device: [Load the Wi-Fi/BT firmware and setup BT on the end device](#load-the-wi-fibt-firmware). Set up the OpenThread Border Router by following the instructions for eithe [otbr-agent or otbr-agent-iwxxx](#setup-otbr). While setting up the Border Router, comment out the command line that connects to a Wi-Fi SSID (i.e., $ wpa_supplicant -d -B -i mlan0 -c ./wifiap.conf) to avoid conflicts.**
...
<img src="../../_static/images/mpu/matter_demos/onnetwork.png" width = "500"/>
...
**Note: Before running imx-thread-br-app, ensure the following prerequisites are completed on the end device: Set up the Open Thread Border Router by following the instructions for either [otbr-agent or otbr-agent-iwxxx](#setup-otbr)**
...
To run applications with trusty on the i.MX 8M Mini EVK by on-network commissioning way, you should use to the [enable command](#enable-the-secure-storage-service) to enable the secure storage service.

<a name="running-matter-ota"></a>

## Running Matter OTA for Wi-Fi/BT firmware and application on the i.MX MPU platform

Matter provides a chip-ota-provider-app and chip-ota-requestor-app to support the OTA update feature. On the i.MX MPU platform, we provide a reference for 88W8987, IW612 firmware, and nxp-thermostat-app OTA update. The network topology diagram as is shown below.

<img src="../../_static/images/mpu/matter_demos/ota.png" width = "500"/>
